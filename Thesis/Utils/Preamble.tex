...
\hypersetup{
  ...
  pdfkeywords = {Artificial Intelligence, Machine Learning, Deep Learning,
...
\def\drop #1#2 #3 #4 {% note the space before {
  \lettrine{#1}{#2 #3 #4} % a trailing space
}
...
%   "\pi" and "e" constants typesetted where they belong (vertically).
\makeatletter
\AddEverypageHook{%
  \if@mainmatter%
  \ifnum\value{page} = 1 % This corresponds to page 2.
  \begin{textblock*}{.4cm}[0, .5](.05\paperwidth, .7182818284590452\paperheight)
    \noindent\resizebox{.4cm}{!}{\(e\)}
  \end{textblock*}
  \fi
  \ifnum\value{page} = 2 % This corresponds to page 3.
  \begin{textblock*}{.4cm}[1, .5](.95\paperwidth, .1415926535897932\paperheight)
    \noindent\resizebox{.4cm}{!}{\(\pi\)}
  \end{textblock*}
  \fi
  \fi
}
