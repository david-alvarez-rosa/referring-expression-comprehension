...
\epigraphhead[75]{
  \epigraph{\itshape{}Begin at the beginning, the King said gravely, ``and go
    on till you come to the end: then stop.''}
  {---Lewis \textsc{Carroll} \\
    \textit{Alice in Wonderland}}}


\lettrine{A}{rtificial Intelligence} (\acs{ai}\glsunset{ai})\index{Artificial
  intelligence} is one of the branches of \gls{cs} that is most fashionable in
recent years%
...
\gls{ai}. \myCite{nilsson09:quest_artif_intel}, however, provides a useful
definition.
...
broad world of \gls{ai} is the scope of \gls{ml}\index{Machine learning}, that
...
Within this scope appear the \glspl{ann}, which are the basis of a large family
...
at the intersection between \gls{cv} and \gls{nlp}.
...
mask of the segmentation, a bounding box\index{Bounding box}.
...
\begin{remarkBox}
  ...
  \citeauthor*{contributor18:artif_intel_will_replac_tasks_not_jobs}~\cite{contributor18:artif_intel_will_replac_tasks_not_jobs},
...
\subsubsection{Home Automation and \glsentryshort{iot}}
...
\begin{figure}[ht]
  ...
  \caption[Examples of applications in robotics]{Examples of applications in
    ...
    \figcite{iam21:futur_mater_handl} (right).}%
  \label{fig:robot}
\end{figure}
...
\begin{description}
  ...
  displayed graphically. \chapRef{cha:files}
