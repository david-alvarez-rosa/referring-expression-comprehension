...
finally, we will make an exhaustive study of \gls{sota} works in this area by
reviewing the more recent literature (\vref{sec:sota}).
...
\begin{figure}[ht]
  ...
  \caption[Examples from RefCOCO and RefCOCO+ datasets]{Examples from RefCOCO
    ...
    \captioncite{kazemzadeh14:refer_game}.}\label{fig:refcoco}
\end{figure}
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
Es decir, si \(p = 1\), la función de pérdida será \(0\) if and only if
\(\hat{p} = 1\) y será más grande cuanto más diferentes sean \(p\) y
\(\hat{p}\). La penalización crecerá exponencialmente hasta hacerse infinita
para el valor \(\hat{p} = 0\). El caso \(p = 0\) es simétrico.
...
\begin{itemize}
  ...
  \item \textbf{\gls*{dnc}}. Esta función de pérdida, introducida por
  \myCite{ronneberger15:u}, fuerza la separación entre objetos contiguos. Es
  similar a \gls{bce}, pero con un término adiccional de distancia entre
  objetos,
  \begin{equation}
    \text{\gls*{dnc}}(p, \hat{p}) =
  ...
  \end{equation}
  Aquí \(d_1(p)\) denota la distancia al borde de la nearest cell y \(d_2(p)\)
  la distancia al borde de la second nearest cell. El resto son hiperparámetros
  de la función de loss.\footnote{Los autores usan en sus experimentos \(w_0 =
    10\) y \(\sigma \approx 5\) (see Section~3 from \cite{ronneberger15:u}).}
\end{itemize}
...
será usado para la evaluación del modelo en la \vref{sec:quantitative-eval}.
...
Ver primero \vref{sec:quantitative-measures} para una explicación detallada del
índice de Jaccard o \gls{iou}, que es una medida cuantitativa ampliamente
utilizada como técnica de evaluación.

\begin{remarkBox}
  La razón por la cual no se puede usar directamente \gls{iou} como función de
  pérdida es que es infactible la optimización en el caso de non-overlapping
  bounding boxes (ya que, en este caso, \gls{iou} has no value and therefore no
  gradient).
\end{remarkBox}

\myCite{rezatofighi19:gener} resuelven este problema introduciendo una loss
function basada en el \gls{iou} y a la que llaman \gls{giou}. Esta
generalización garantiza la existencia de gradiente en todos los casos y, por
tanto, la hace adecuada para usarse en un proceso de optimización.

\begin{figure}[ht]
  \centering
  \includegraphics[width=.65\textwidth]{Images/GIoU.png}
  \caption[\acl*{giou} algorithm]{\acf{giou} algorithm general aplicado a
    conjuntos multi-dimensionales arbitrarios. En el caso concreto de \gls{rec}
    serán bounding boxes en \(\R^2\). From
    \captioncite{rezatofighi19:gener}.}\label{fig:giou}
\end{figure}

Esta loss function viene resumida en la \vref{fig:giou}. Es una generalización
que conserva las propiedas relevantes del \gls{iou}, pero que corrige los
problemas relacionados con su diferenciabilidad.
...
En cualquier ámbito de \gls{ml} será necesario de disponer de técnicas de
evaluación para poder decidir si los resultados obtenidos con el modelo son
suficientemente buenos y también para poder realizar comparaciones.

Las técnicas más útiles suelen ser métricas de caracter cuantitativo, las
cuales se discutirán en la \vref{sec:quantitative-measures}. Además, en el caso
concreto de \gls{rec}, debido al uso de imágnes, será posible realizar también
una evaluación visual/cualitativa (ver \vref{sec:qualitative-eval}).
...
related to the calculation of \gls{iou} or Jaccard index. Este índice se
fundamenta en los conceptos de intersección y de unión entre la segmentación
predecida (que es una máscara \emph{binaria}) y el ground truth (en la
\vref{fig:sets} se muestran unos diagramas con estos conceptos).
...
\begin{figure}[ht]
  ...
  \caption[Union and intersection of sets \(A\) and \(B\)]{Graphic
    representation of union and intersection of sets \(A\) and \(B\). From
    \captioncite{contributors21:jaccar} (both).}\label{fig:sets}
\end{figure}
...
\begin{figure}[ht]
  ...
  \caption[Explicación del Jaccard Index]{Explicación y ejemplo del Jaccard
    Index en el caso de bounding boxes. From
    \captioncite{contributors21:jaccar} (both).}
\end{figure}
...
\begin{figure}[ht]
  ...
  \caption[Multimodal embedding technique]{Multimodal embedding into
    ...
    \captioncite{cornia18:towar_cycle_consis_model_text_image_retriev}.}\label{fig:joint}
\end{figure}
...
\begin{figure}[ht]
  ...
  \caption[\acl*{mattnet}]{\gls{mattnet}: given an expression, it is divided
    ...
    \captioncite{yu18:mattn}.}\label{fig:mattnet}
\end{figure}
...
\begin{figure}[ht]
  ...
  \caption[Graph based model representation]{Summary representation of
    ...
    expression. From \captioncite{qiao20:refer_expres_compr}.}\label{fig:graph}
\end{figure}
