...
\begin{figure}[ht]
  ...
  \caption[Graph based model representation]{Summary representation of
    graphic-based models. From the image, the grain representation is built,
...
responsible for generating the graphical representation of the image (the nodes
it generates will be the candidate objects). Finally, the matching module is
the one that computes the matching score between \gls{re} and object (for each
of the candidate objects).

Other authors in exploiting the graphs in this context are
\myCite{yang19:dynam}, who create the model \gls{dga}, which allows multi-step
