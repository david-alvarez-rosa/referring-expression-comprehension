...
\begin{quoteBox}
  ...
  \tcblower
  ---\myCiteShort{caudill87:neural_networ_primer_part_i}
\end{quoteBox}
...
be a binary operation for real numbers. Then, the element-wise operation is
defined as follows,
\begin{equation}
...
\subsection{\acl*{fnn}} \label{sec:fnn}
...
As we discussed in the topology section (\ref{sec:topology-fcnn}), all neurons
...
\subsubsection{Trainning: backpropagation} \label{sec:backward}
...
below in \vref{sec:topology-fcnn-1}.
...
\subsection{\acl*{rnn}} \label{sec:rnn}
...
such as: image captioning (\texttt{one to many}), action prediction
(\texttt{many to one}), video captioning (\texttt{many to many} first option),
and video classification on frame label (\texttt{many to many} second
option). The extreme case of \texttt{one to one} corresponds precisely to a
normal fully connected layered neural network such as the one studied in
\vref{sec:fnn}.
...
\begin{quoteBox}
  ...
  \tcblower
  ---\myCiteShort{vaswani17:atten_all_you_need}
\end{quoteBox}
...
\(\mathcal{L} (\Omega, \theta)\) and we will assume that it is continuosly
...
\begin{equation}
  \hat{\theta} = \argmin_{\theta} \mathcal{L}(\Omega, \theta),
\end{equation}
...
values for the parameters such that the loss function corresponds to a
(global) minimum.
...
that if \(\hat{\theta}\) is a local minimum of \(\mathcal{L}\), then
\(\nabla\mathcal{L}(\hat{\theta}) = 0\), therefore the optimization methods
will focus in finding stationary points (that, hopefully, correspond to global
or local minimums).\footnote{These are called first-order optimization methods
  ...
  \(\mathcal{L}\) and in \gls{dl} is computationally expensive.}

\subsubsection{Optimization methods}
Gradient descent, then stochastic gradient descent and then momentum.
