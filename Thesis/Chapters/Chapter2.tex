...
\begin{itemize}
  ...
  current \acl{sota} \footnote{In \today, current date of the document.}. They
...
\begin{equation}
 ...
 \(\mathbf{W}^l\). This is, \((\mathbf{W} l) ij = w ij ^ l\).
...
\vref{eq:math-conv}. In practice, the input images\footnote{At levels deeper
  than a \gls{cnn} we will stop understanding the layers as images and call
  them feature maps} are three-dimensional tensors for the images \acs{rgb}. In
convolutional layers, a two-dimensional convolution is made with a
three-dimensional filter (also called kernel) on each channel of the input
